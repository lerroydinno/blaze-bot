...
(async function() {
    ...
    class BlazeWebSocket {
        ...
        doubleTick(cb) {
            ...
            this.ws.onclose = () => { clearInterval(this.ping); console.warn('WebSocket fechado'); };
            this.ws.onerror = () => console.warn('WebSocket erro');
        }
    }
    ...
    class BlazeInterface {
        ...
        async makePrediction() {
            ...
            const probs = arr[0], mx = Math.max(...probs), idx = probs.indexOf(mx);
        ...
        renderPrediction() {
            const p = this.nextPred, col=['Branco','Vermelho','Preto'][p.color];
        ...
        compareWinLose() {
            ...
            document.getElementById('winLose').textContent = win
                ? `✅ Ganhou (${['B','V','P'][this.nextPred.color]})`
                : `❌ Perdeu (${['B','V','P'][this.nextPred.color]})`;
            document.getElementById('result').textContent =
                `Último: ${['Branco','Vermelho','Preto'][this.lastResult.color]} ${this.lastResult.roll}`;
        }

        updateStats(r) {
            const c = this.stats.counts; c[r.color]++;
            this.stats.sequences.push(r.color);
            if (this.stats.sequences.length > 3) {
                const seq = this.stats.sequences.slice(-3).join('');
                // ex: detect 3 pretos seguidos: '222'
            }
            if (r.color === 0) {
        ...
        renderStats() {
            ...
            document.getElementById('stats').innerHTML =
                `B:${c[0]} V:${c[1]} P:${c[2]}<br>Int Branco(s): ${wi}`;
        }
    ...
    setInterval(async()=>{
        if (!ws.ws || ws.ws.readyState!==1) {
            const r = await app.fallbackFetch();
            app.onNewResult(r);
        }
    },5000);
