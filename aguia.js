(async function () {
  ...
  function getRollFromHash(hash) {
    const number = parseInt(hash.slice(0, 8), 16) % 15; // Garante 0-14
    const tile = TILES.find(t => t.number === number);
    const cor = tile.color === "white" ? "BRANCO" : tile.color === "red" ? "VERMELHO" : "PRETO";
    console.log(`Hash: ${hash.slice(0, 8)}, Number: ${number}, Cor: ${cor}`); // Depuração
    return { cor, numero: tile.number };
  }

  // Função para mapear o roll da API diretamente
  function getRollFromAPI(colorValue, roll) {
    const cor = colorValue === 0 ? "BRANCO" : colorValue <= 7 ? "VERMELHO" : "PRETO";
    console.log(`API Color: ${colorValue}, Roll: ${roll}, Cor: ${cor}`); // Depuração
    return { cor, numero: roll };
  }
  ...
  async function gerarPrevisao(serverSeed, clientSeed, hist = []) {
    ...
    console.log(`Previsão: ${previsao.cor} (${previsao.numero}), Confiança: ${confianca}%`); // Depuração
  ...
  setInterval(async () => {
    try {
      ...
      const ultimo = data[0];
      const corNum = Number(ultimo.color); // Valor da API (0 = branco, 1-7 = vermelho, 8-14 = preto)
      const roll = Number(ultimo.roll); // Número do roll da API
      const cor = corNum === 0 ? "BRANCO" : corNum <= 7 ? "VERMELHO" : "PRETO";
      const numero = roll; // Usar o roll da API como número
