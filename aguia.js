...
class BlazeInterface {
  ...
  predictWhiteColor() {
    ...
    const whiteCount = lastResults.filter(r => r.color === 0).length;
    const probability = (whiteCount / lastResults.length) * 100;

    // Prioriza previsão para branco quando a probabilidade for acima de 70%
    if (probability >= 70) {
      return { color: 0, colorName: 'Branco', isWaiting: false };
    }
    return null;
  }
