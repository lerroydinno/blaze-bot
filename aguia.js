...
class BlazeWebSocket {
    ...
    doubleTick(cb) {
        ...
        this.ws.onopen = () => {
            ...
            this.pingInterval = setInterval(() => this.ws.send('2'), 25000); // Ping para manter a conexão ativa
        };
...
class BlazeInterface {
    constructor() {
        ...
        this.correctPredictions = 0; // Contador de acertos
        this.totalPredictions = 0;   // Contador total de previsões
        this.initMonitorInterface(); // Inicia o painel (login removido)
    }
    ...
    initMonitorInterface() {
        ...
        this.overlay.innerHTML = `
            <div class="blaze-monitor" id="blazeMonitorBox">
                <div style="display:flex;justify-content:space-between;align-items:center">
                    <h3 style="margin:0">Previsão de Cor</h3>
        ...
        this.ws.doubleTick((data) => {
            this.processResults(data);
        });
    }

    processResults(data) {
        const { color, roll, status } = data;
        this.totalPredictions++;

        // Exemplo de previsão baseada no histórico
        let predictedColor = color === 'red' ? 'Vermelho' : (color === 'black' ? 'Preto' : 'Branco');
        let accuracy = ((Math.random() * 100).toFixed(2)); // Exemplo de taxa de acerto

        // Atualiza a previsão no painel
        document.getElementById('blazePrediction').innerHTML = `
            <div class="prediction-title">Próxima Cor Prevista</div>
            <div class="prediction-value">
                <span class="color-dot ${color === 'red' ? 'color-dot-1' : (color === 'black' ? 'color-dot-2' : 'color-dot-0')}"></span>
                ${predictedColor}
            </div>
            <div class="prediction-accuracy">Taxa de Acerto: ${accuracy}%</div>
        `;
    }
}

// Inicializa o painel diretamente (sem login)
new BlazeInterface();
