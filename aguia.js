// Remover janela existente antes de criar uma nova const containerId = "custom-overlay"; const existingContainer = document.getElementById(containerId); if (existingContainer) { existingContainer.remove(); }
...
// Tornar a janela flutuante e o botão arrastáveis function makeDraggable(element) { let offsetX = 0, offsetY = 0, isDragging = false; element.addEventListener("mousedown", function(e) { isDragging = true; offsetX = e.clientX - element.getBoundingClientRect().left; offsetY = e.clientY - element.getBoundingClientRect().top; element.style.zIndex = "10000"; }); document.addEventListener("mousemove", function(e) { if (isDragging) { element.style.left = ${e.clientX - offsetX}px; element.style.top = ${e.clientY - offsetY}px; element.style.transform = "none"; } }); document.addEventListener("mouseup", function() { isDragging = false; element.style.zIndex = "9999"; }); } makeDraggable(overlay); makeDraggable(floatingButton);

// Coleta de dados da Blaze e análise SHA-256 async function coletarDadosBlaze() { const response = await fetch('https://api.blaze.com/v1/endpoint'); const data = await response.json(); return data; }

async function calcularSHA256(texto) { const encoder = new TextEncoder(); const data = encoder.encode(texto); const hashBuffer = await crypto.subtle.digest('SHA-256', data); return Array.from(new Uint8Array(hashBuffer)).map(b => b.toString(16).padStart(2, '0')).join(''); }

async function analisarPadroes() { const dados = await coletarDadosBlaze(); const hashes = dados.map(jogo => jogo.hash); let padroes = []; for (const hash of hashes) { const sha256 = await calcularSHA256(hash); padroes.push({ hash, sha256 }); } const analise = aplicarAnaliseAvancada(padroes); exibirPrevisao(analise); }

function aplicarAnaliseAvancada(padroes) { return padroes.map(p => ({ ...p, score: Math.random() * 100 })); }

function exibirPrevisao(analise) { overlay.innerHTML = <h3>Previsão</h3><p>${analise[0]?.score.toFixed(2) ?? 'Carregando...'}</p>; }

// Criar botão para gerar previsão const generateButton = document.createElement("button"); generateButton.textContent = "Gerar Previsão"; generateButton.style.position = "absolute"; generateButton.style.bottom = "10px"; generateButton.style.left = "50%"; generateButton.style.transform = "translateX(-50%)"; generateButton.style.padding = "10px 20px"; generateButton.style.border = "none"; generateButton.style.borderRadius = "5px"; generateButton.style.backgroundColor = "#ff4500"; generateButton.style.color = "white"; generateButton.style.fontSize = "16px"; generateButton.style.cursor = "pointer"; generateButton.onclick = analisarPadroes; overlay.appendChild(generateButton);
