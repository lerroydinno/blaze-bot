// === Blaze Double com Painel Original e Análises Avançadas ===
...
this.overlay.innerHTML = `
  <div class="blaze-monitor" id="blazeMonitorBox">
    <h3>App SHA256</h3>
    <button id="blazeMinBtn" class="blaze-min-btn">&minus;</button>
...
predictNextColor() { if (!this.results.length) return null; const last = this.results.find(r => r.status === 'complete'); if (!last) return null;

const reds = this.results.filter(r => r.color === 1).length;
const blacks = this.results.filter(r => r.color === 2).length;
const whites = this.results.filter(r => r.color === 0).length;

const total = reds + blacks + whites;
const predColor = reds > blacks ? 1 : 2;

return {
  color: predColor,
  colorName: predColor === 1 ? 'Vermelho' : 'Preto',
  isWaiting: this.results.some(r => r.status === 'waiting')
};

}

updatePredictionStats(cur) { if (this.results.length < 2 || cur.status !== 'complete') return; const prev = this.results.filter(r => r.status === 'complete')[1]; if (!prev) return; this.totalPredictions++; if (prev.color === cur.color) this.correctPredictions++; }

updateResults(d) { const id = d.id || tmp-${Date.now()}-${d.color}-${d.roll}; const i = this.results.findIndex(r => (r.id || r.tmp) === id); if (i >= 0) this.results[i] = { ...this.results[i], ...d }; else { if (this.results.length > 50) this.results.pop(); this.results.unshift({ ...d, tmp: id }); if (d.status === 'complete') this.updatePredictionStats(d); }
...
showNotification(d, win) { document.querySelectorAll('.blaze-notification').forEach(n => n.remove()); const n = document.createElement('div'); n.className = blaze-notification ${win ? 'notification-win' : 'notification-loss'}; n.textContent = ${win ? 'GANHOU' : 'PERDEU'}! ${(d.color === 0 ? 'BRANCO' : d.color === 1 ? 'VERMELHO' : 'PRETO')} ${d.roll ?? ''}; document.body.appendChild(n); setTimeout(() => n.classList.add('show'), 50); setTimeout(() => { n.classList.remove('show'); setTimeout(() => n.remove(), 300); }, 3000); } }
