...
// Função para calcular SHA-256 e gerar previsão async function gerarPrevisao() { try { const response = await fetch("https://blaze.bet/api/double/recent"); const data = await response.json(); const lastRoll = data[0].roll;
...
async function atualizarJanela() { const previsao = await gerarPrevisao(); overlay.innerHTML = <h3>Status do Jogo</h3> <p id='resultado'>Carregando...</p> <h4>Previsão para esta rodada:</h4> <div id='previsao' style='text-align:center; font-size: 20px; padding: 10px; border-radius: 5px; background: ${previsao === "Branco" ? "white" : previsao.toLowerCase()}; color: ${previsao === "Branco" ? "black" : "white"};'>${previsao}</div> <button id='gerarPrevisao' style='width: 100%; padding: 10px; margin-top: 10px; background: blue; color: white; border: none; border-radius: 5px; cursor: pointer;'>Gerar Nova Previsão</button>; document.getElementById('gerarPrevisao').onclick = atualizarJanela; setTimeout(atualizarResultado, 5000); }

async function atualizarResultado() { try { const response = await fetch("https://blaze.bet/api/double/recent"); const data = await response.json(); const resultado = data[0].color === "red" ? "Vermelho" : data[0].color === "black" ? "Preto" : "Branco";
