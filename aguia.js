...
class BlazeInterface {
  ...
  loadHistory() {
    ...
    if (saved) {
      try {
        ...
        console.log('[Blaze] Histórico carregado:', this.results);
      } catch (e) {
  ...
  saveHistory() {
    localStorage.setItem('blaze_history', JSON.stringify(this.results));
  }

  injectGlobalStyles() {
    const style = document.createElement('style');
    style.textContent = `.blaze-bubble{position:fixed;bottom:20px;right:20px;width:60px;height:60px;border-radius:50%;background:url('https://aguia-gold.com/static/logo_blaze.jpg') center/cover no-repeat, rgba(34,34,34,.92);box-shadow:0 4px 12px rgba(0,0,0,.5);cursor:pointer;z-index:10000;display:none;}
  ...
  updateResults(d) {
    ...
    if (this.results.filter(r => r.status === 'complete').length >= 10) {
  ...
  timePrediction() {
    ...
    const history = this.results.filter(r => r.status === 'complete' && new Date(r.time || Date.now()).getHours() === hour);
